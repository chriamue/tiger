...
inline void tiny_quantized_deconv2d_kernel(const deconv_params &params,
  ...
  for (serial_size_t inc = 0; inc < params.in.depth; inc++) {
    for (serial_size_t ins = 0; ins < params.in.height * params.in.height;
         ins++) {
  ...
  for (serial_size_t inc = 0; inc < params.in.depth; inc++) {
    for (serial_size_t ins = 0;
         ins < params.weight.height * params.weight.height; ins++) {
      serial_size_t idx = params.in.get_index(0, 0, inc);
  ...
  if (params.has_bias) {
    for (serial_size_t inc = 0; inc < params.out.depth; inc++) {
  ...
  for_i(layer_parallelize, params.out.depth, [&](int o) {
    for (serial_size_t inc = 0; inc < params.in.depth; inc++) {
      ...
      for (serial_size_t y = 0; y < params.in.height; y++) {
        for (serial_size_t x = 0; x < params.in.width; x++) {
          ...
          const uint8_t *ppi = pi + y * params.in.width + x;
          // should be optimized for small kernel(3x3,5x5)
          for (serial_size_t wy = 0; wy < params.weight.height; wy++) {
            for (serial_size_t wx = 0; wx < params.weight.width; wx++) {
              pout_quantized[(y * params.h_stride + wy) * params.out.width +
                             (x * params.w_stride + wx)] +=
                static_cast<int32_t>(ppw[wy * params.weight.width + wx] -
                                     offset_filter) *
    ...
    if (params.has_bias) {
      ...
      int32_t *ppout_quantized =
        pout_quantized + params.out.width * params.out.height;
...
inline void tiny_quantized_deconv2d_back_kernel(const deconv_params &params,
  ...
  for (serial_size_t inc = 0; inc < params.out.depth; inc++) {
    for (serial_size_t ins = 0; ins < params.out.height * params.out.height;
         ins++) {
      ...
      for (serial_size_t y = 0; y < params.in.height; y++) {
        for (serial_size_t x = 0; x < params.in.width; x++) {
          ...
          int32_t *ppdelta_quantized_dst =
            pdelta_quantized_dst + y * params.in.width + x;
            ...
            for (serial_size_t wx = 0; wx < params.weight.width; wx++) {
              sum +=
                static_cast<int32_t>(ppw[wy * params.weight.width + wx] -
                                     offset_filter) *
                static_cast<int32_t>(
                  pdelta_src[(y * params.h_stride + wy) * params.out.width +
                             (x * params.w_stride + wx)] -
                  offset_curr_delta);
            }
          }
      ...
      for (serial_size_t wy = 0; wy < params.weight.height; wy++) {
        for (serial_size_t wx = 0; wx < params.weight.width; wx++) {
          ...
          for (serial_size_t y = 0; y < params.in.height; y++) {
            for (serial_size_t x = 0; x < params.in.width; x++) {
              dst +=
                (static_cast<int32_t>(*(prevo + y * params.in.width + x)) -
                 offset_prev_out) *
                (static_cast<int32_t>(*(delta + y * params.out.width + x)) -
                 offset_curr_delta);
            }
          }
