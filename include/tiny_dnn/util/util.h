...
U rescale(T x, T src_min, T src_max, U dst_min, U dst_max) {
	U value = static_cast<U>(((x - src_min) * (dst_max - dst_min))
...
Stream &operator<<(Stream &s, const index3d<T> &d) {
	s << d.width << "x" << d.height << "x" << d.depth;
	return s;
}
...
inline serial_size_t conv_out_length(serial_size_t in_length,
		serial_size_t window_size, serial_size_t stride, padding pad_type) {
...
struct are_all<checker, T0, Ts...> : std::integral_constant<bool,
		checker<T0>::value && are_all<checker, Ts...>::value> {
};
